...
# Difference from `best_program.py`:

- Do not allow to have 0 replica so that no expert will be dropped.
"""
...
def replicate_experts(
    ...
    # Ensure each logical expert gets at least one replica (rank=0)
    # Initialize counts and next ranks to 1 for all experts after the base assignment.
    current_logcnt = torch.ones(num_layers, num_log, dtype=torch.int64, device=device)
    current_replica_rank_to_assign = torch.ones(num_layers, num_log, dtype=torch.int64, device=device)

    base_log_indices = torch.arange(num_log, dtype=torch.int64, device=device).unsqueeze(0).expand(num_layers, -1)
    phy2log[:, :num_log] = base_log_indices
    rank[:, :num_log] = torch.zeros(num_layers, num_log, dtype=torch.int64, device=device)

    # Score for assigning an additional replica to each expert
    ...
    # Fill the remaining physical expert slots greedily
    for k_phy_idx in range(num_log, num_phy):
...
def rebalance_experts(
    ...
    max_log_replicas = int(logcnt.max().item())
