...
def replicate_experts(
    ...
    # Pre-allocate a 1-hot tensor for scatter_add_ operations.
    # This tensor will have shape [num_layers, 1] and contains ones.
    ones_for_scatter = torch.ones(num_layers, 1, dtype=torch.int64, device=device)
    ...
    for k_phy_idx in range(num_phy):
        ...
        # Unsqueeze best_log_indices once for repeated use in gather/scatter operations.
        best_log_indices_unsqueezed = best_log_indices.unsqueeze(1)
        ...
        # `current_replica_rank_to_assign` (gathered using `best_log_indices_unsqueezed`) provides the
        ...
        rank[:, k_phy_idx] = current_replica_rank_to_assign.gather(
            1, best_log_indices_unsqueezed).squeeze(1)

        # Update `current_logcnt` for the selected experts (increment by 1).
        current_logcnt.scatter_add_(1, best_log_indices_unsqueezed, ones_for_scatter)
        ...
        current_replica_rank_to_assign.scatter_add_(
            1, best_log_indices_unsqueezed, ones_for_scatter)
        ...
        selected_weights = weight.gather(1, best_log_indices_unsqueezed).squeeze(1)
        # Gather the new (incremented) counts for the selected experts.
        updated_counts = current_logcnt.gather(1, best_log_indices_unsqueezed).squeeze(1).float()
        ...
        next_avg_load_score.scatter_(1, best_log_indices_unsqueezed, new_scores.unsqueeze(1))
